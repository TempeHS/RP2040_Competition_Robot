...
# Every turn reuses the SAME gyro turn PID you wrote in Challenge 4 —
# it is HELD, so keep the C4 gain values.
#
...
side_Ki = 0.0  # integral gain
side_Kd = 0.0  # derivative gain
side_INTEGRAL_MAX = 0  # anti-windup clamp

FRONT_SLOW_DISTANCE = 0  # start slowing when a wall is this close ahead
FRONT_Kp = 0.0  # how hard to slow down on approach

# --- TURN parameters (the gyro turn PID from Challenge 4 — you set the gains) ---
...
# --- NIB_WALL parameters (you set) ---
NIB_FORWARD_BEFORE = 0.0  # seconds to drive forward (NO PID) BEFORE the turn
NIB_FORWARD_AFTER = 0.0  # seconds to drive forward (NO PID) AFTER the turn

# --- Trigger thresholds (the logic that moves between states) ---
FRONT_STOP_DISTANCE = 0  # a front wall this close = reached -> TURN (you set)
