...
function runOnce({ spawnX, subSteps, dtTotal, label }) {
  ...
  globalThis.__aidriverModuleFactory = function (name) {
    ...
    mod.hold_state = new Sk.builtin.func(function (seconds) {
      ...
      if ((r.collisionCount || 0) > 0) {
        stop = "collision";
        throw new Sk.builtin.SystemExit(new Sk.builtin.str("__"));
      }
      if (r.y < 200) {
        stop = "success";
        throw new Sk.builtin.SystemExit(new Sk.builtin.str("__"));
  ...
  return Sk.misceval
    .asyncToPromise(() => Sk.importMainWithBody("<probe>", false, code, true))
    .catch((e) => {
      if (!String(e).includes("__test_stop_") && !String(e).includes("__"))
        throw e;
    })
    .then(() => {
      let minX = Infinity,
      ...
      console.log(
        `${label.padEnd(40)} stop=${stop} ticks=${samples.length} xRange=[${minX.toFixed(1)},${maxX.toFixed(1)}] maxH=${maxAbsH.toFixed(2)}° finalY=${f.y.toFixed(1)}`,
      );
    });
}
...
(async () => {
  ...
  for (const x of [200, 220, 240, 250, 260, 280, 300, 320, 350, 400]) {
    await runOnce({
      ...
      subSteps: 1,
      dtTotal: 0.05,
      label: `spawn x=${x}, dt=0.05 single-step`,
    });
  }
  console.log("---");
