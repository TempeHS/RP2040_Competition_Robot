"""MicroPython driver for the Grove / STMicro VL53L0X Time-of-Flight sensor.

Vendored from kevinmcaleer/vl53l0x (itself derived from uceeatz/VL53L0X),
with an added ``set_address()`` method so multiple sensors can share one I2C
bus. All VL53L0X sensors power up at address 0x29, so to run two of them you
hold each sensor's XSHUT pin low, then bring them up one at a time and call
``set_address()`` before enabling the next one.
"""
...
class VL53L0X:
    ...
    def set_address(self, new_address):
        """Change this sensor's 7-bit I2C address (default 0x29).

        Used to run multiple VL53L0X sensors on one bus: bring up one sensor
        at a time (others held in reset via their XSHUT pins) and give each a
        unique address before enabling the next. The change is volatile and
        resets to 0x29 on power cycle, so re-run this at every boot.
        """
        new_address &= 0x7F
