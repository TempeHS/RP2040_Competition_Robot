...
def announce(*lines):
    ...
    This is what lets you *watch the code and the robot together*: every stage
    prints its banner and mirrors it on the SSD1306 OLED, so the screen always
    names the action the robot is performing right now. Each OLED line shows up
    to 16 characters and only the first four lines are sent to the display. If
    no OLED is fitted, show_display() is a harmless no-op and only the console
    prints.
    """
    for line in lines:
        print(line)
